...
Explanatory notes are within audit scope when they assert the Greek's meaning,
the translation's logic, or factual context used to justify a rendering. A
materially false note may be reported even when the verse itself is correct.
...
- `FORMULA`: a registered or repeated Homeric formula is materially mishandled
  in meaning or rhetorical function. Mechanical fixed-rendering conformance is
  audited separately and is outside the reviewer finding count.
...
The automated formula audit owns departures from fixed registered wording,
including semantically defensible variants. Do not report a finding solely
because wording differs from the fixed register. Reviewers should report a
registered or repeated formula only when the English also causes a material
semantic or rhetorical loss that the mechanical comparison cannot decide.
Describe that loss directly and do not duplicate an automated formula-policy
violation.
